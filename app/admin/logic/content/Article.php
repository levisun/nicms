...
class Article extends BaseLogic
{
    ...
    public function query(): array
    {
        ...
        if ($query_page > $this->cache->get('admin content article last_page' . $query_limit, $query_page)) {
            return [
                'debug' => false,
                'cache' => true,
                'msg'   => 'error',
        ...
        $total = $this->cache->get('admin content article total', false);
        ...
        if (!$this->cache->has('admin content article total')) {
            $this->cache->set('admin content article total', $list['total'], 28800);
        }

        if (!$this->cache->has('admin content article last_page' . $query_limit)) {
            $this->cache->set('admin content article last_page' . $query_limit, $list['last_page'], 28800);
        }
