...
class Link extends BaseLogic
{
    ...
    public function query(): array
    {
        ...
        if ($query_page > $this->cache->get('admin content link last_page' . $query_limit, $query_page)) {
            return [
                'debug' => false,
                'cache' => true,
                'msg'   => 'error',
        ...
        $total = $this->cache->get('admin content link total', false);
        ...
        if (!$this->cache->has('admin content link total')) {
            $this->cache->set('admin content link total', $list['total'], 28800);
        }

        if (!$this->cache->has('admin content link last_page' . $query_limit)) {
            $this->cache->set('admin content link last_page' . $query_limit, $list['last_page'], 28800);
        }
