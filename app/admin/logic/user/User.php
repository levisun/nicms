...
class User extends BaseLogic
{
    ...
    public function query(): array
    {
        ...
        if ($query_page > $this->cache->get('admin user user last_page' . $query_limit, $query_page)) {
            return [
                'debug' => false,
                'cache' => true,
                'msg'   => 'error',
        ...
        $total = $this->cache->get('admin user user total', false);
        ...
        if (!$this->cache->has('admin user user total')) {
            $this->cache->set('admin user user total', $list['total'], 28800);
        }

        if (!$this->cache->has('admin user user last_page' . $query_limit)) {
            $this->cache->set('admin user user last_page' . $query_limit, $list['last_page'], 28800);
        }
