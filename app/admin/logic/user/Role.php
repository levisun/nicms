...
class Role extends BaseLogic
{
    ...
    public function query(): array
    {
        ...
        if ($query_page > $this->cache->get('admin user role last_page' . $query_limit, $query_page)) {
            return [
                'debug' => false,
                'cache' => true,
                'msg'   => 'error',
        ...
        $total = $this->cache->get('admin user role total', false);
        ...
        if (!$this->cache->has('admin user role total')) {
            $this->cache->set('admin user role total', $list['total'], 28800);
        }

        if (!$this->cache->has('admin user role last_page' . $query_limit)) {
            $this->cache->set('admin user role last_page' . $query_limit, $list['last_page'], 28800);
        }
