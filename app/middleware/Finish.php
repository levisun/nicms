...
class Finish
{
    ...
    {
        // 千分几率跳转至并发错误页
        if (rand(1, 999) === 1 && !in_array(Request::controller(true), ['abort', 'admin', 'api'])) {
            Log::record('[并发]' . Request::ip(), 'alert');
            $url = url('error');
            throw new \think\exception\HttpResponseException(redirect($url));
        }

        $dir = app()->getRuntimePath() . 'concurrent' . DIRECTORY_SEPARATOR;
        if (!is_dir($dir)) {
            chmod(app()->getRuntimePath(), 0777);
            mkdir($dir, 0777, true);
        }
        $dir .= md5(__DIR__ . Request::ip() . date('Ymd')) . '.php';
        ...
        if (rand(1, 3) == 1) {
            ...
            if (is_array($result)) {
                ...
                file_put_contents($dir, '<?php return ' . var_export($result, true) . ';');
            }
            ...
            if (is_file($dir)) {
                ...
                if (!empty($data[$time]) && $data[$time] >= 20) {
                    file_put_contents($dir . '.lock', date('YmdHis'));
                }
            }
        }
