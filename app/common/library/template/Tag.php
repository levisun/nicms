...
class Tag
{
        ...
        if (!empty($_attr)) {
            ...
            $parseStr .= '->find();
                if ($result && $result = $result->toArray()):
                    $result["thumb"] = \app\common\library\Image::path($result["thumb"]);
    ...
    public function head(array $_attr, string &$_content)
    {
        ...
        return '<!DOCTYPE html>' .
            ...
            ('admin' !== app('http')->getName() ? '<script src="__API_HOST__tools/record.do?token=<?php echo md5(request()->url(true));?>" async></script>' : '') .
